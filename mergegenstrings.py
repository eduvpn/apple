...
def merge(merged_fname, old_fname, new_fname):
    try:
        ...
        print 'Error: input files have invalid format.', sys.exc_info()[0]
...
def localizeCode(path, routine):
    ...
    languages = [lang for lang in [os.path.join(path + '/Resources', name) for name in os.listdir(path + '/Resources')]
...
if __name__ == '__main__':
	argc = len(argv)
	if (argc <= 1 or 5 < argc):
		print 'Usage: %s path_to_source_directory [routine] [development_language] [all|interface_only|code_only]' % argv[0]
		quit()

	path = os.path.abspath(argv[1])
	routine = argv[2] if argc > 2 else 'NSLocalizedString'
	developmentLanguage = argv[3] if argc > 3 else 'en'
	option = argv[4] if argc > 4 else 'all'
	print option
	if (option == 'all') or (option == 'code_only'):
		localizeCode(path, routine)
	if (option == 'all') or (option == 'interface_only'):
		localizeInterface(path + '/Resources', developmentLanguage)
